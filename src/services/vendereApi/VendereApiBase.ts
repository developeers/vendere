...
const config: AxiosRequestConfig = {
  baseURL:
    "https://firestore.googleapis.com/v1/projects/vendere-23f76/databases/(default)/documents/",
};
const queryApiConfig: AxiosRequestConfig = {
  baseURL:
    "https://firestore.googleapis.com/v1/projects/vendere-23f76/databases/(default)/documents:runQuery",
};
...
const interceptAxiosResponseError = async (error: AxiosError) => {
  if (error.response?.status == httpStatusCodes.UNAUTHORIZED) {
    ...
    await axios
      .post<any, RefreshAccessTokenApiResponse>(refreshAccessTokenApiUrl, {
...
VendereApiInstance.interceptors.response.use(
  undefined,
  interceptAxiosResponseError
);
VendereQueryApiInstance.interceptors.response.use(
  undefined,
  interceptAxiosResponseError
);
